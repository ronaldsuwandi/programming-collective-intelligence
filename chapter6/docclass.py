...
class Classifier:
    ...
    def weighted_probability(self, feature, category, probability_fn, weight=1.0,
        ...
        total_appeared = sum([self.feature_count(feature, category) for category in self.categories()])

        # Calculate the weighted average
        return ((weight * assumed_probability) + (total_appeared * prob)) / (weight+total_appeared)
